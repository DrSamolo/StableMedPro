-- =============================================================================
-- STEP 5.11 - delete_user_secure: reassign audit logs before profile deletion
-- =============================================================================
...
BEGIN
  ...
  IF p_reassign_user_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    RAISE EXCEPTION 'Reassignation obligatoire pour supprimer un utilisateur';
  END IF;
  ...
  GET DIAGNOSTICS moved_audit_logs = ROW_COUNT;
  ...
  IF deleted_auth_users <> 1 THEN
    RAISE EXCEPTION 'Suppression impossible: compte auth introuvable ou deja supprime';
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'moved_audit_logs', moved_audit_logs,
