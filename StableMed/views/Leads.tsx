...
import { Lead, Training, Comment, Profile, Team } from '../types';
...
const Leads: React.FC = () => {
  ...
  const fetchLeads = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('leads')
        .select(`*, profiles:user_id ( id, full_name, avatar_url, role, team_id )`)
      ...
      try {
        ...
        mapAndSetLeads(legacyData);
      } catch (finalError) {
         console.error('Critical Leads Error', finalError);
         setSchemaError(true);
      }
    } finally {
      setLoading(false);
  ...
  const handleImportCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      ...
      if (newLeads.length > 0) {
        ...
        else { addNotification('success', 'Import réussi'); fetchLeads(); }
      }
  ...
  const handleConvertToDeal = async () => {
    ...
    if (selectedTrainingIds.length > 0) {
        ...
        ).catch(() => console.warn('Deal Trainings table missing'));
    }
