...
import { notifySlackDealWon } from '../lib/integrations';
...
const Pipeline: React.FC = () => {
  ...
  const fetchDeals = async () => {
    ...
    try {
        ...
        const mappedDeals: Deal[] = (data || []).map((d: any) => ({
            ...
            owner_id: d.owner_id,
            assignee: d.owner, // Joined Profile
  ...
  const createDeal = async () => {
    ...
    if (!error && dealData) {
        if (selectedTrainingIds.length > 0 && !compatibilityMode) {
            ...
            await supabase.from('deal_trainings').insert(relations).catch(e => console.warn("Relation insert failed", e));
        }
        fetchDeals();
  ...
  const handleUpdateDeal = async () => {
    ...
    try {
        ...
        if (!compatibilityMode) {
            await supabase.from('deal_trainings').delete().eq('deal_id', selectedDeal.id).catch(() => {});
  ...
  const handleConfirmWin = async () => {
      ...
      notifySlackDealWon(dealToWin, salesPersonName);
      ...
      setDealToWin(null);
      addNotification('success', 'Félicitations ! Vente enregistrée et notifiée sur Slack.');
  };
